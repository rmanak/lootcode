...
| `scripts/seed.py`, `audit.py`, `verify_bank.py`, `check_constraint_validators.py`, `strengthen_tests.py`, `import_collection.py`, `verify_json.py`, `figures.py` | Operational scripts: seed/verify the DB, statement↔test↔judge consistency audit, run every canonical against its tests, audit per-problem input validators, machine-generate discriminating hidden tests, bulk-import a staged collection, batch-verify loose problem JSON before importing, and SVG figure helpers. |
| `tests/` | pytest suite. |
